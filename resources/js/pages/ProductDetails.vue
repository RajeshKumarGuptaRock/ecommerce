                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    <div v-for="size in product.sizes" :key="size.id" class="relative">
                                        ...
                                        <label :for="'size-' + size.id"
                                            class="min-w-10 h-9 flex justify-center items-center border-2 border-slate-200 dark:border-slate-700 rounded-lg cursor-pointer peer-checked:border-primary peer-checked:bg-primary-50 px-3 text-sm font-medium text-slate-700 dark:text-slate-300 transition">
                                            {{ size.name }}
                                        </label>
                                    </div>
                                </div>
...
const findProductInCart = (productId) => {
    ...
    basketStore.products.forEach((item) => {
        item.products.find((p) => {
            if (p.id == productId) return (foundProduct = p);
        });
    });
